...
class SatelliteChannel:
    ...
    def print_link_budget(
        ...
        """打印链路预算表"""
        budget = self.get_link_budget(distance_km, elevation_deg, link_type)
        
        print("\n" + "=" * 60)
        print(f"         链路预算 ({link_type.upper()})")
        print("=" * 60)
        print(f"  距离:           {budget['distance_km']:.1f} km")
        print(f"  仰角:           {budget['elevation_deg']:.1f}°")
        print("-" * 60)
        print(f"  发射功率:       {budget['tx_power_dbm']:.1f} dBm")
        print(f"  发射天线增益:   {budget['tx_antenna_gain_db']:.1f} dB")
        print(f"  接收天线增益:   {budget['rx_antenna_gain_db']:.1f} dB")
        print("-" * 60)
        print(f"  自由空间损耗:   {budget['fspl_db']:.1f} dB")
        print(f"  大气损耗:       {budget['atmospheric_loss_db']:.1f} dB")
        print(f"  雨衰:           {budget['rain_attenuation_db']:.1f} dB")
        print(f"  总路径损耗:     {budget['total_path_loss_db']:.1f} dB")
        print("-" * 60)
        print(f"  噪声功率:       {budget['noise_power_dbm']:.1f} dBm")
        print(f"  SNR:            {budget['snr_db']:.1f} dB")
        print("-" * 60)
        print(f"  信道容量:       {budget['channel_capacity_mbps']:.2f} Mbps")
        print(f"  传播时延:       {budget['propagation_delay_ms']:.2f} ms")
        print("=" * 60)
...
class MultiUserChannel:
    ...
    def __init__(
        ...
        Args:
            config: 信道配置
            total_bandwidth_mhz: 卫星总带宽 (MHz)
            max_users_per_satellite: 每颗卫星最大服务用户数
        """
        self.channel = SatelliteChannel(config)
        self.total_bandwidth_mhz = total_bandwidth_mhz
        self.max_users = max_users_per_satellite
    
    def allocate_bandwidth_equal(self, num_users: int) -> float:
        """
        平均分配带宽
        ...
        """
        if num_users <= 0:
            return self.total_bandwidth_mhz
        
        return self.total_bandwidth_mhz / num_users
    
    def allocate_bandwidth_proportional(
        self,
        ...
        Args:
            user_demands: 用户需求数组 (任意单位，用于计算比例)
            min_bandwidth_mhz: 最小保障带宽
            
        Returns:
            每用户分配带宽数组 (MHz)
        """
        num_users = len(user_demands)
        
        if num_users == 0:
            return np.array([])
        
        # 计算比例
        total_demand = np.sum(user_demands)
        if total_demand <= 0:
            return np.full(num_users, self.total_bandwidth_mhz / num_users)
        
        proportions = user_demands / total_demand
        
        # 分配带宽
        allocated = proportions * self.total_bandwidth_mhz
        
        # 确保最小带宽
        allocated = np.maximum(allocated, min_bandwidth_mhz)
        
        # 如果超出总带宽，按比例缩减
        if np.sum(allocated) > self.total_bandwidth_mhz:
            allocated = allocated * self.total_bandwidth_mhz / np.sum(allocated)
    ...
    def compute_user_rates(
        ...
        allocated_bandwidths_mhz: np.ndarray,
        link_type: str = 'uplink'
    ) -> np.ndarray:
        """
        计算多用户的数据速率
        ...
        Args:
            distances_km: 用户到卫星距离数组
            elevations_deg: 用户仰角数组
            allocated_bandwidths_mhz: 分配给每用户的带宽数组
            link_type: 链路类型
            
        Returns:
            每用户数据速率数组 (Mbps)
        """
        num_users = len(distances_km)
        rates = np.zeros(num_users)
        
        for i in range(num_users):
            rates[i] = self.channel.compute_data_rate_mbps(
                distances_km[i],
                elevations_deg[i],
                link_type,
                allocated_bandwidths_mhz[i]
            )
...
# 便捷函数
def create_default_channel() -> SatelliteChannel:
    """创建默认配置的信道模型"""
    return SatelliteChannel(ChannelConfig())


def compute_link_capacity(
    distance_km: float,
    elevation_deg: float,
    ...
    return channel.compute_data_rate_mbps(
        distance_km, elevation_deg, 'uplink', bandwidth_mhz
    )
