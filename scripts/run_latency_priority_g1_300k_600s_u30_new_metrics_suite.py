"""Run the u30 latency-priority suite with the new metric protocol.

This mirrors run_latency_priority_g1_300k_600s_u10_suite.py, but uses the
merged HAN+Attn method as the system run and evaluates it with the current
paper-style metrics:
...
The result layout is separated from older metric protocols:

    results/full_train_latency_priority_g1_300k_600s_u30_new_metrics_<run_id>
    results/baseline_compare/g1_300k_600s_u30_new_metrics_<run_id>

The default comparison covers the four learned MAPPO-family methods plus
simple rule-based baselines:

    HAN+Attn, HAN+MAPPO, Attn+MAPPO, MAPPO, random, min_distance,
    full_local, joint_greedy
"""
...
RUN_LABEL = "g1_300k_600s_u30_new_metrics"
DEFAULT_BASELINES = (
    "han_mappo",
    "mappo_no_han",
    "attn_mappo",
...
class SuiteConfig:
    ...
    exp_name: str = "han_attn_latency_priority_g1_300k_600s_u30_new_metrics"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the u30 new-metric HAN+Attn latency-priority run, then compare "
            "it with HAN+MAPPO, Attn+MAPPO, MAPPO, and rule-based baselines."
        )
    )
    parser.add_argument(
        "--run-id",
    ...
    parser.add_argument(
        ...
        help=(
            "Baselines for comparison. Defaults to HAN+MAPPO, MAPPO, "
            "Attn+MAPPO, and simple rule-based baselines."
        ),
    )
